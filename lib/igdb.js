...
const getCoverImg = async (gameName) => {
  ...
  const body = `search "${gameName}"; fields cover.image_id, name; limit 5;`;
  const headers = {
  ...
  return axios
    ...
    .then((response) => {
      //Based on the search results, find the exact match.
      const exactMatchImgId = response.data.find(game => game.name.toLowerCase() === gameName.toLowerCase()).cover.image_id;
      // extract cover img id from response - OLD
      //var coverImgId = response.data[0].cover.image_id;

      // put the img ID into URL so we can send it back
      var coverImageUrl = `https://images.igdb.com/igdb/image/upload/t_cover_big/${exactMatchImgId}.jpg`;
