...
import addCompletedGame from "@schemas/addCompletedGame";
const validate = ajv.compile(addCompletedGame);
import { getCoverImg } from "@/lib/igdb";

// @route    POST api/gamesCompleted/addGame
...
export const POST = async (req) => {
  ...
  try {
    ...
    // if the gameList already has 100 games is what we set it for now, return an error
    if (gameList.games.length >= 100) {
      return NextResponse.json(
        { errors: ["You have reached max limit of 100 games."] },
        { status: 400 }
      );
    }
    ...
    const newGame = {
      ...
      rating: body.rating,
      currentlyPlaying: body.currentlyPlaying || false
    };
